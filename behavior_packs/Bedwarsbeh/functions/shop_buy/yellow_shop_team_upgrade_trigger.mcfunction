...
execute @s[scores={"分队"=3},l=99] ~~~ tellraw @s { "rawtext" : [ { "text" : "§lShop >> §cYou don't have enough levels." } ] }
...
execute @a[tag=upgrade_yellow_trigger] ~~~ tellraw @s { "rawtext" : [ { "text" : "§lShop >> §d§lYou purchased the §r§6team TRIGGER" } ] }
execute @a[tag=upgrade_yellow_trigger] ~~~ tellraw @a[scores={"分队"=3}] { "rawtext" : [ { "text" : "§lShop · Team Upgrade >> §e " } ,{ "selector" :  "@a[tag=want_upgrade_trigger_yellow]"},{ "text" : "§f§l purchased the team TRIGGER" }] }
