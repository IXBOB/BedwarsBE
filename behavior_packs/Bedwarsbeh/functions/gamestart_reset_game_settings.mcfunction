...
#倒计时0 title
execute @e[type=armor_stand,name=main,scores={starting=1,"开始倒计时"=0}] ~~~ title @a title §a§lGAME START
#倒计时0 subtitle
execute @e[type=armor_stand,name=main,scores={starting=1,"开始倒计时"=0}] ~~~ titleraw @a subtitle { "rawtext" : [{"translate":"%%2","with":{"rawtext":[{"selector":"@e[type=armor_stand,name=main,scores={游戏模式=2}]"},{"text":"§o§lRUSH MODE"},{"text":"§r§f§lNORMAL MODE"}]}} ] }
#红床存活 -> 1
...
#set事件倒计时120
execute @e[type=armor_stand,name=main,scores={starting=1,"开始倒计时"=0}] ~~~ scoreboard players set "§eNEXT EVENT" "游戏显示" 121
#放置红石线
...
#tellraw获得无敌时间
execute @e[type=armor_stand,name=main,scores={starting=1,"开始倒计时"=0}] ~~~ execute @a[scores={"分队"=1..2}] ~~~ tellraw @s { "rawtext" : [ { "text" : "§f§lBED WARS >> §b§lYou gained 3 seconds of invincibility" } ] }
#击杀数 -> 0
...
#清空当局经验和当局金币
execute @e[type=armor_stand,name=main,scores={starting=1,"开始倒计时"=0}] ~~~ scoreboard players set @a "当局经验" 0
execute @e[type=armor_stand,name=main,scores={starting=1,"开始倒计时"=0}] ~~~ scoreboard players set @a "当局硬币" 0
#生成game_uid并赋予玩家
